...
处理：在读到第一个非标识符字符时停止（不消耗该字符），然后检查关键字表
```
...
**与实现中的错误处理关系**：本图仅描述合法数字/科学计数法的识别路径，指数缺失、非法小数点等错误情况由第 4.3.1 小节中的 `read_number` 实现负责检测，并通过统一错误处理机制上报。
...
**与实现中的错误处理关系**：本图同样只展示正常闭合的字符串路径，未闭合字符串或字符串中出现换行等错误情况由第 4.3.2 小节中的 `read_string` 实现负责检测，并通过统一错误处理机制上报。
...
def add_to_constant_table(self, value: Union[int, float, str], const_type: str) -> int:
    # 常数表去重并返回索引
    # 去重策略：按“类型 + 值”作为键，严格区分 int 1 与 float 1.0
    key = (const_type, value)
...
> 复杂度来源说明（简述）：
>
> - 时间 O(N)：单指针顺序扫描，字符仅被读取/前瞻/消费常数次；各识别过程按最长匹配线性推进，无全局回退。
> - 空间 O(M+K)：仅存储去重后的标识符与常量表条目；运行时除当前词素缓冲与少量状态外为 O(1)。更精确地可写作 O(Σ|id|+Σ|const|)。
> - 符号表查找 O(1)：基于哈希字典，平均常数时间；重复标识符不重复入表。
> - 关键字检查 O(1)：关键字集合为哈希表，读取出词素后常数时间判定/映射（读取本身的代价已计入 O(N)）。
